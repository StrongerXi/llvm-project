...
//
static inline void
RemapInstruction(Instruction *I, std::map<const Value *, Value *> &ValueMap) {
...
bool InlineMethod(BasicBlock::InstListType::iterator CIIt) {
  ...
  // std::cerr << "Inlining " << CalledMeth->getName() << " into "
  //      << CurrentMeth->getName() << std::endl;
  ...
  for (Method::BasicBlocksType::const_iterator BI =
    ...
    Instruction *NewInst;
    for (BasicBlock::InstListType::const_iterator II =
    ...
    default:
      std::cerr << "MethodInlining: Don't know how to handle terminator: "
                << TI;
      abort();
    }
  }
...
bool InlineMethod(CallInst *CI) {
  ...
  BasicBlock::InstListType::iterator CallIt =
      find(PBB->getInstList().begin(), PBB->getInstList().end(), CI);
