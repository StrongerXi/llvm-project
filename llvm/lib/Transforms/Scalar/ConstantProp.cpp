...
inline static bool ConstantFoldUnaryInst(Method *M, Method::inst_iterator &DI,
  ...
  if (!ReplaceWith)
    return false; // Nothing new to change...
  ...
  if (Op->hasName())
    ReplaceWith->setName(Op->getName());
...
inline static bool ConstantFoldBinaryInst(Method *M, Method::inst_iterator &DI,
                                          BinaryOperator *Op, ConstPoolVal *D1,
                                          ConstPoolVal *D2) {
  ConstPoolVal *ReplaceWith = 0;
...
// them together...
//
inline static bool ConstantFoldInstruction(Method *M,
                                           Method::inst_iterator &II) {
  Instruction *Inst = *II;
  if (Inst->isBinaryOp()) {
    Value *D1, *D2;
    ...
    PHINode *PN =
        (PHINode *)Inst; // If it's a PHI node and only has one operand
    ...
    if (PN->getOperand(1) == 0) { // If the PHI Node has exactly 1 operand
      ...
      if (PN->hasName())
        V->setName(PN->getName()); // Inherit PHINode name
