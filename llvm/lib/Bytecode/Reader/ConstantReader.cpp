...
bool BytecodeParser::parseTypeConstant(const uchar *&Buf, const uchar *EndBuf,
  ...
  unsigned PrimType;
  if (read_vbr(Buf, EndBuf, PrimType))
    return true;
  ...
  case Type::ArrayTyID: {
    ...
    int NumElements;
    if (read_vbr(Buf, EndBuf, NumElements))
      return true;
...
bool BytecodeParser::parseConstPoolValue(const uchar *&Buf, const uchar *EndBuf,
  ...
  case Type::UIntTyID: {
    ...
    if (read_vbr(Buf, EndBuf, Val))
      return true;
    if (!ConstPoolUInt::isValueValidForType(Ty, Val))
      return true;
  ...
  case Type::IntTyID: {
    ...
    if (read_vbr(Buf, EndBuf, Val))
      return true;
    if (!ConstPoolSInt::isValueValidForType(Ty, Val))
      return 0;
  ...
  case Type::TypeTyID:
    if (parseTypeConstant(Buf, EndBuf, V))
      return true;
    break;
  ...
  case Type::ArrayTyID: {
    ...
    std::vector<ConstPoolVal *> Elements;
...
bool BytecodeParser::ParseConstantPool(const uchar *&Buf, const uchar *EndBuf,
  ...
  while (Buf < EndBuf) {
    ...
    for (unsigned i = 0; i < NumEntries; i++) {
      ConstPoolVal *I;
      if (parseConstPoolValue(Buf, EndBuf, Ty, I))
        return true;
#if 0
      std::cerr << "  Read const value: <" << I->getType()->getName() 
	   << ">: " << I->getStrValue() << endl;
#endif
      insertValue(I, Tab);
      CP.insert(I);
