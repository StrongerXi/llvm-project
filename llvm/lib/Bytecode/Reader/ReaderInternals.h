...
private:
  ...
  bool insertValue(Value *val, std::vector<ValueList> &vl);
...
static inline unsigned getValueIDNumberFromPlaceHolder(Value *Def) {
  ...
  case Type::LabelTyID:
    return ((BBPHolder *)Def)->getID();
  case Type::MethodTyID:
    return ((MethPHolder *)Def)->getID();
  default:
    return ((DefPHolder *)Def)->getID();
  }
}
...
static inline bool readBlock(const uchar *&Buf, const uchar *EndBuf,
  ...
  cerr << "StartLoc = " << ((unsigned)Buf & 4095) << " Type = " << Type
       << " Size = " << Size << endl;
  return Result;
#else
