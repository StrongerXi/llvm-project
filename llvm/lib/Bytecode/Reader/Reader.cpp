...
bool BytecodeParser::getTypeSlot(const Type *Ty, unsigned &Slot) {
  if (Ty->isPrimitiveType()) {
    ...
    if (I == TypeMap.end())
      return true; // Didn't find type!
    Slot = I->second;
  }
  // std::cerr << "getTypeSlot '" << Ty->getName() << "' = " << Slot <<
  // std::endl;
  return false;
}
...
const Type *BytecodeParser::getType(unsigned ID) {
  ...
  // std::cerr << "Looking up Type ID: " << ID << std::endl;
...
bool BytecodeParser::insertValue(Value *Def, std::vector<ValueList> &ValueTab) {
  unsigned type;
  if (getTypeSlot(Def->getType(), type))
    return true;

  if (ValueTab.size() <= type)
    ValueTab.resize(type + 1, ValueList());

  // std::cerr << "insertValue Values[" << type << "][" << ValueTab[type].size()
  //      << "] = " << Def << std::endl;
...
Value *BytecodeParser::getValue(const Type *Ty, unsigned oNum, bool Create) {
  ...
  case Type::MethodTyID:
    ...
    return d;
  default:
    d = new DefPHolder(Ty, oNum);
    break;
  }

  assert(d != 0 && "How did we not make something?");
  if (insertValue(d, LateResolveValues))
    return 0;
  return d;
}
...
bool BytecodeParser::ParseSymbolTable(const uchar *&Buf, const uchar *EndBuf) {
  while (Buf < EndBuf) {
    ...
    if (read_vbr(Buf, EndBuf, NumEntries) || read_vbr(Buf, EndBuf, Typ))
      return true;
  ...
  while (Buf < EndBuf) {
    ...
    case BytecodeFormat::ConstantPool:
      if (ParseConstantPool(Buf, Buf + Size, M->getConstantPool(), Values)) {
        std::cerr << "Error reading constant pool!\n";
        delete M;
        return true;
      }
      break;
    ...
    case BytecodeFormat::BasicBlock: {
      BasicBlock *BB;
      if (ParseBasicBlock(Buf, Buf + Size, BB) || insertValue(BB, Values)) {
        std::cerr << "Error parsing basic block!\n";
        delete M;
    ...
    case BytecodeFormat::SymbolTable:
      if (ParseSymbolTable(Buf, Buf + Size)) {
        std::cerr << "Error reading method symbol table!\n";
        delete M;
        return true;
      }
      break;
...
bool BytecodeParser::ParseModuleGlobalInfo(const uchar *&Buf, const uchar *End,
  ...
  while (MethSignature != Type::VoidTyID) { // List is terminated by Void
    ...
    unsigned TypeSlot;
    if (getTypeSlot(Def->getType(), TypeSlot))
      return true;
    ...
    MethodSignatureList.push_back(
        std::make_pair((const MethodType *)Ty, SlotNo));
    if (read_vbr(Buf, End, MethSignature))
      return true;
  }
...
bool BytecodeParser::ParseModule(const uchar *Buf, const uchar *EndBuf,
  ...
  if (read_vbr(Buf, EndBuf, FirstDerivedTyID))
    return true;
  if (align32(Buf, EndBuf))
    return true;
  ...
  while (Buf < EndBuf) {
    ...
    case BytecodeFormat::ModuleGlobalInfo:
      if (ParseModuleGlobalInfo(Buf, Buf + Size, C)) {
        std::cerr << "Error reading class global info section!\n";
        delete C;
        return true;
      }
      break;
    ...
    case BytecodeFormat::ConstantPool:
      if (ParseConstantPool(Buf, Buf + Size, C->getConstantPool(),
                            ModuleValues)) {
        std::cerr << "Error reading class constant pool!\n";
        delete C;
        return true;
      }
      break;
    ...
    case BytecodeFormat::Method: {
      if (ParseMethod(Buf, Buf + Size, C)) {
        delete C;
        return true; // Error parsing method
      }
      break;
    }
    ...
    case BytecodeFormat::SymbolTable:
      if (ParseSymbolTable(Buf, Buf + Size)) {
        std::cerr << "Error reading class symbol table!\n";
        delete C;
        return true;
      }
      break;
    ...
    default:
      std::cerr << "Unknown class block: " << Type << std::endl;
      Buf += Size;
      if (OldBuf > Buf)
        return true; // Wrap around!
      break;
...
Module *BytecodeParser::ParseBytecode(const uchar *Buf, const uchar *EndBuf) {
  ...
  Module *Result;
  if (ParseModule(Buf, EndBuf, Result))
    return 0;
  return Result;
}
...
Module *ParseBytecodeFile(const std::string &Filename) {
  ...
  if (Filename != std::string("-")) { // Read from a file...
    ...
    uchar *Buffer = (uchar *)mmap(0, Length, PROT_READ, MAP_PRIVATE, FD, 0);
    if (Buffer == (uchar *)-1) {
      close(FD);
      return 0;
    }
