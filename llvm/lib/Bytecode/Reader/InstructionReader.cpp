...
bool BytecodeParser::ParseRawInst(const uchar *&Buf, const uchar *EndBuf,
  ...
  case 0:
    ...
    default:
      ...
      // Allocate a std::vector to hold arguments 3, 4, 5, 6 ...
  ...
  // std::cerr << "NO: "  << Result.NumOperands   << " opcode: " <<
  // Result.Opcode
...
bool BytecodeParser::ParseInstruction(const uchar *&Buf, const uchar *EndBuf,
  ...
  if (Raw.Opcode >= Instruction::FirstUnaryOp &&
    ...
    case 0:
      std::cerr << "Invalid phi node encountered!\n";
      delete PN;
      return true;
    case 1:
    ...
    case 3:
      PN->addIncoming(getValue(Raw.Ty, Raw.Arg1));
      PN->addIncoming(getValue(Raw.Ty, Raw.Arg2));
      PN->addIncoming(getValue(Raw.Ty, Raw.Arg3));
      break;
    default:
      PN->addIncoming(getValue(Raw.Ty, Raw.Arg1));
      PN->addIncoming(getValue(Raw.Ty, Raw.Arg2));
    ...
    if (Raw.NumOperands == 0) {
      ...
      Res = new ReturnInst(getValue(Raw.Ty, Raw.Arg1));
      return false;
    }
    ...
    if (Raw.NumOperands < 3)
      return false; // No destinations?  Wierd.
    ...
    switch (Raw.NumOperands) {
    case 0:
      std::cerr << "Invalid call instruction encountered!\n";
      return true;
    case 1:
      break;
    case 2:
    ...
    case 3:
      ...
      if (It == PL.end())
        return true;
      Params.push_back(getValue(*It++, Raw.Arg3));
      break;
    default:
      Params.push_back(getValue(*It++, Raw.Arg2));
      {
  ...
  std::cerr << "Unrecognized instruction! " << Raw.Opcode << std::endl;
  return true;
}
