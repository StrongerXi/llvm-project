...
inline bool ModuleAnalyzer::handleType(std::set<const Type *> &TypeSet,
                                       const Type *T) {
  if (!T->isDerivedType())
    return false; // Boring boring types...
  if (TypeSet.count(T) != 0)
    return false;    // Already found this type...
...
bool ModuleAnalyzer::processConstPool(const ConstantPool &CP, bool isMethod) {
  ...
  for (ConstantPool::plane_const_iterator PI = CP.begin(); PI != CP.end();
       ++PI) {
    const ConstantPool::PlaneType &Plane = **PI;
    if (Plane.empty())
      continue; // Skip empty type planes...

    if (processConstPoolPlane(CP, Plane, isMethod))
      return true;
    ...
    for (ConstantPool::PlaneType::const_iterator CI = Plane.begin();
      ...
      if ((*CI)->getType() == Type::TypeTy)
        if (handleType(TypeSet, ((const ConstPoolType *)(*CI))->getValue()))
          return true;
      if (handleType(TypeSet, (*CI)->getType()))
        return true;

      if (processConstant(*CI))
        return true;
    }
  }
  ...
  if (!isMethod) {
    ...
    for (Module::MethodListType::const_iterator I = M->getMethodList().begin();
      ...
      if (handleType(TypeSet, (*I)->getType()))
        return true;
      if (visitMethod(*I))
        return true;
    }
...
bool ModuleAnalyzer::processBasicBlock(const BasicBlock *BB) {
  ...
  for (; Inst != BB->getInstList().end(); Inst++) {
    if (preProcessInstruction(*Inst) || processInstruction(*Inst))
      return true;
  }
  return false;
}
