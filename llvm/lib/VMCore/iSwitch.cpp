...
SwitchInst::SwitchInst(Value *V, BasicBlock *DefV)
    : TerminatorInst(Instruction::Switch), DefaultDest(DefV, this),
      Val(V, this) {
  assert(Val && DefV);
}
...
void SwitchInst::dest_push_back(ConstPoolVal *OnVal, BasicBlock *Dest) {
  Destinations.push_back(
      dest_value(ConstPoolUse(OnVal, this), BasicBlockUse(Dest, this)));
}
...
unsigned SwitchInst::getNumOperands() const { return 2 + Destinations.size(); }
...
const Value *SwitchInst::getOperand(unsigned i) const {
  ...
  if (i & 1)
    return Destinations[slot].second;
