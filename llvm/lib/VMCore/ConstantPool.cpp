...
bool ConstantPool::remove(ConstPoolVal *N) {
  unsigned Ty = N->getType()->getUniqueID();
  if (Ty >= Planes.size())
    return true; // Doesn't contain any of that type

  PlaneType::iterator I = std::find(Planes[Ty]->begin(), Planes[Ty]->end(), N);
  if (I == Planes[Ty]->end())
    return true;
  Planes[Ty]->remove(I);
  return false;
...
void ConstantPool::dropAllReferences() {
  for (unsigned i = 0; i < Planes.size(); i++)
    for (PlaneType::iterator I = Planes[i]->begin(); I != Planes[i]->end(); I++)
      (*I)->dropAllReferences();
}
...
struct EqualsConstant {
  ...
  inline bool operator()(const ConstPoolVal *V) const { return v->equals(V); }
};

ConstPoolVal *ConstantPool::find(const ConstPoolVal *V) {
  const PlaneType *P;
  if (getPlane(V->getType(), P))
    return 0;
...
void ConstPoolVal::setName(const std::string &name) {
  SymTabValue *P;
...
ConstPoolVal *ConstPoolVal::getNullConstant(const Type *Ty) {
  ...
  case Type::BoolTyID:
    return new ConstPoolBool(false);
  case Type::SByteTyID:
  case Type::ShortTyID:
  case Type::IntTyID:
  case Type::LongTyID:
    return new ConstPoolSInt(Ty, 0);

  case Type::UByteTyID:
  case Type::UShortTyID:
  case Type::UIntTyID:
  case Type::ULongTyID:
    return new ConstPoolUInt(Ty, 0);

  case Type::FloatTyID:
  case Type::DoubleTyID:
    return new ConstPoolFP(Ty, 0);
  default:
    return 0;
  }
}
...
ConstPoolBool::ConstPoolBool(bool V, const std::string &Name)
    : ConstPoolVal(Type::BoolTy, Name) {
  Val = V;
}

ConstPoolSInt::ConstPoolSInt(const Type *Ty, int64_t V, const std::string &Name)
...
ConstPoolUInt::ConstPoolUInt(const Type *Ty, uint64_t V,
...
ConstPoolFP::ConstPoolFP(const Type *Ty, double V, const std::string &Name)
    : ConstPoolVal(Ty, Name) {
  assert(isValueValidForType(Ty, V) && "Value to large for type!");
  Val = V;
}

ConstPoolType::ConstPoolType(const Type *V, const std::string &Name)
    : ConstPoolVal(Type::TypeTy, Name), Val(V) {}

ConstPoolArray::ConstPoolArray(const ArrayType *T,
                               std::vector<ConstPoolVal *> &V,
                               const std::string &Name)
    : ConstPoolVal(T, Name) {
...
std::string ConstPoolBool::getStrValue() const {
  if (Val)
    return "true";
  else
    return "false";
...
std::string ConstPoolSInt::getStrValue() const { return itostr(Val); }

std::string ConstPoolUInt::getStrValue() const { return utostr(Val); }

std::string ConstPoolFP::getStrValue() const {
  assert(0 && "FP Constants Not implemented yet!!!!!!!!!!!");
  return "% FP Constants NI!" /* + dtostr(Val)*/;
}

std::string ConstPoolType::getStrValue() const { return Val->getName(); }

std::string ConstPoolArray::getStrValue() const {
  std::string Result = "[";
  if (Val.size()) {
    Result += " " + Val[0]->getType()->getName() + " " + Val[0]->getStrValue();
    for (unsigned i = 1; i < Val.size(); i++)
      Result +=
          ", " + Val[i]->getType()->getName() + " " + Val[i]->getStrValue();
  }

  return Result + " ]";
}

std::string ConstPoolStruct::getStrValue() const {
  std::string Result = "{";
  if (Val.size()) {
    Result += " " + Val[0]->getType()->getName() + " " + Val[0]->getStrValue();
    for (unsigned i = 1; i < Val.size(); i++)
...
bool ConstPoolArray::equals(const ConstPoolVal *V) const {
  ...
  if (Val.size() != AV->Val.size())
    return false;
  for (unsigned i = 0; i < Val.size(); i++)
    if (!Val[i]->equals(AV->Val[i]))
      return false;
...
bool ConstPoolStruct::equals(const ConstPoolVal *V) const {
  ...
  if (Val.size() != SV->Val.size())
    return false;
  for (unsigned i = 0; i < Val.size(); i++)
    if (!Val[i]->equals(SV->Val[i]))
      return false;
