...
public:
  Method(const MethodType *Ty, const std::string &Name = "");
  ~Method();
  ...
  inline const ArgumentListType &getArgumentList() const {
    return ArgumentList;
  }
  ...
  typedef InstIterator<BasicBlocksType, BasicBlocksType::iterator,
                       BasicBlock::InstListType::iterator, Instruction *>
      inst_iterator;
  ...
  inline inst_const_iterator inst_begin() const {
    return inst_const_iterator(*this);
  }
  inline inst_const_iterator inst_end() const {
    return inst_const_iterator(*this, true);
  }
};
