...
public:
  AllocationInst(ConstPoolType *tyVal, Value *arrSize, unsigned iTy,
      ...
      : Instruction(tyVal->getValue(), iTy, Name), TyVal(tyVal, this),
        ArraySize(arrSize, this) {
    ...
    assert(arrSize == 0 ||
           (getType()->getValueType()->isArrayType() &&
            ((const ArrayType *)getType()->getValueType())->isUnsized()) &&
               "Trying to allocate something other than unsized array, with "
               "size!");
  ...
  virtual bool setOperand(unsigned i, Value *Val) {
    if (i == 0) {
      ...
      // Make sure that if a size is specified, that it is a uint!
      assert(Val == 0 ||
             Val->getType() == Type::UIntTy && "Malloc SIZE is not a 'uint'!");
...
public:
  ...
  virtual std::string getOpcode() const { return "malloc"; }
};
...
public:
  ...
  virtual std::string getOpcode() const { return "alloca"; }
};
...
public:
  FreeInst(Value *Ptr, const std::string &Name = "")
      : Instruction(Type::VoidTy, Instruction::Free, Name), Pointer(Ptr, this) {
  ...
  virtual bool setOperand(unsigned i, Value *Val) {
    if (i == 0) {
      assert(!Val ||
             Val->getType()->isPointerType() && "Can't free nonpointer!");
      Pointer = Val;
      return true;
    }
    return false;
  ...
  virtual std::string getOpcode() const { return "free"; }
};
