...
#else
  Result = Buf[0] | (Buf[1] << 8) | (Buf[2] << 16) | (Buf[3] << 24) |
           ((uint64_t)(Buf[4] | (Buf[5] << 8) | (Buf[6] << 16) | (Buf[7] << 24))
            << 32);
#endif
  Buf += 8;
  return false;
...
static inline bool read(const unsigned char *&Buf, const unsigned char *EndBuf,
  ...
  unsigned Size;
  if (read_vbr(Buf, EndBuf, Size))
    return true; // Failure reading size?
  if (Buf + Size > EndBuf)
    return true; // Size invalid?
...
//
static inline void output(unsigned i, std::vector<unsigned char> &Out,
                          int pos = -1) {
#ifdef LITTLE_ENDIAN
  if (pos == -1)
...
static inline void output(int i, std::vector<unsigned char> &Out) {
...
//
static inline void output_vbr(uint64_t i, std::vector<unsigned char> &out) {
  while (1) {
    if (i < 0x80) {                    // done?
...
static inline void output_vbr(unsigned i, std::vector<unsigned char> &out) {
  while (1) {
    if (i < 0x80) {                    // done?
      out.push_back((unsigned char)i); // We know the high bit is clear...
      return;
...
static inline void output_vbr(int64_t i, std::vector<unsigned char> &out) {
  if (i < 0)
...
static inline void output_vbr(int i, std::vector<unsigned char> &out) {
  if (i < 0)
...
// alignment...
//
static inline void align32(std::vector<unsigned char> &Out) {
  int NumPads = (4 - (Out.size() & 3)) & 3; // Bytes to get padding to 32 bits
  while (NumPads--)
    Out.push_back((unsigned char)0xAB);
}

static inline void output(const std::string &s, std::vector<unsigned char> &Out,
                          bool Aligned = true) {
