...
public:
  ...
  virtual std::string getOpcode() const { return "ret"; }
...
public:
  ...
  inline bool isUnconditional() const { return Condition == 0 || !FalseDest; }

  virtual std::string getOpcode() const { return "br"; }
...
public:
  typedef std::pair<ConstPoolUse, BasicBlockUse> dest_value;

private:
  BasicBlockUse DefaultDest;
  Use Val;
  std::vector<dest_value> Destinations;

  SwitchInst(const SwitchInst &RI);

public:
  typedef std::vector<dest_value>::iterator dest_iterator;
  typedef std::vector<dest_value>::const_iterator dest_const_iterator;
  ...
  virtual std::string getOpcode() const { return "switch"; }
  inline Value *getOperand(unsigned i) {
